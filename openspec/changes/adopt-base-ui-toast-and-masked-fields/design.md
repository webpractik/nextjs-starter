...
The registry relies on `shimmer`, `scroll-fade-x`, and hidden-scrollbar utilities. Add the exact current `shadcn` release as a root development dependency and import `shadcn/tailwind.css` from the existing global Tailwind entrypoint, following the documented shadcn installation model. This avoids inventing partial local copies and keeps the shared utilities updateable; the CSS is compiled during the build and is not a runtime server dependency.
